...
<div class="flex justify-between items-end mb-4">
  ...
  <form method="GET" id="filterForm" class="flex gap-4">
    <input type="date" name="start_date" id="start_date"
           value="{{ request('start_date') }}" class="border p-2">
    <input type="date" name="end_date" id="end_date"
           value="{{ request('end_date') }}" class="border p-2">
...
function exportPdf() {
    ...
    let url = "{{ route('finance.laporan.pengeluaran.pdf') }}";

    if (start && end) {
        const confirmRange = confirm(
            "Ekspor laporan PENGELUARAN dengan rentang tanggal?\n\n" +
